...
test_GT_data_norm = Function_mk3.normalize_data(test_GT_data)
for i in range(len(test_GT_data)):
    ...
    norm_error_map = np.squeeze(Function_mk3.normalize_data([error_map]))

    accuracy = 100*(1-abs(test_GT_data_norm_aver - test_output_aver)/test_GT_data_norm_aver)
    print(accuracy)
    test_surface = test_data[i][:1000, :3]

    # model actor
    model_actor = Functions.point_actor(test_surface, model_point_size, 'None')
    Functions.assign_value_to_polydata(model_actor.GetMapper().GetInput(), test_output)
    ...
    # gt_actor
    error_actor = Functions.point_actor(test_surface, gt_point_size, 'None')
    Functions.assign_value_to_polydata(error_actor.GetMapper().GetInput(), norm_error_map)
    error_actor.SetPosition(position, 0, 0)
    ...
    writer.SetFileName('./test_result/test_37_model_Net_23_Seq_3_Ch_256_Ker_1/norm error map/'
                       + str(i+1) + '_'
