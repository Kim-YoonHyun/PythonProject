...
def get_model(input_tensor, is_training, bn_decay=None):
    ...
    out3_3 = tf_util.conv2d(out3_2, 64, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
    ...
    adj = tf_util.pairwise_distance(net_3)
    nn_idx = tf_util.knn(adj, k=k)
    edge_feature = tf_util.get_edge_feature(net_3, nn_idx=nn_idx, k=k)

    out4_1 = tf_util.conv2d(edge_feature, 64, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='four/adj_conv1', bn_decay=bn_decay, is_dist=True)

    out4_2 = tf_util.conv2d(out4_1, 64, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='four/adj_conv2', bn_decay=bn_decay, is_dist=True)

    out4_3 = tf_util.conv2d(out4_2, 64, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
    ...
    adj = tf_util.pairwise_distance(net_4)
    nn_idx = tf_util.knn(adj, k=k)
    edge_feature = tf_util.get_edge_feature(net_3, nn_idx=nn_idx, k=k)

    out5_1 = tf_util.conv2d(edge_feature, 64, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='five/adj_conv1', bn_decay=bn_decay, is_dist=True)

    out5_2 = tf_util.conv2d(out5_1, 64, [1, 1],
                            padding='VALID', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='five/adj_conv2', bn_decay=bn_decay, is_dist=True)

    net_5 = tf.reduce_max(out5_2, axis=-2, keepdims=True)

    out7 = tf_util.conv2d(tf.concat([net_1, net_2, net_3, net_4, net_5], axis=-1), 1024, [1, 1],
                          padding='VALID', stride=[1, 1],
