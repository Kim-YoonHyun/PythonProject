...
def get_model(input_tensor, is_training, bn_decay=None):
    ...
    first_CNN_channels = [128, 128, 128]
    second_CNN_channels = [128, 128, 128]
    third_CNN_channels = [128, 128, 128]
    fourth_CNN_channels = [128, 128, 128]
    multi = 2
    ...
    out1_1 = tf_util.conv2d(input_tensor_transformed, first_CNN_channels[0], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='one/adj_conv1', bn_decay=bn_decay, is_dist=True)

    out1_2 = tf_util.conv2d(out1_1, first_CNN_channels[1], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='one/adj_conv2', bn_decay=bn_decay, is_dist=True)

    out1_3 = tf_util.conv2d(out1_2, first_CNN_channels[2], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
    ...
    zero_net_1 = tf.concat(axis=-1, values=[first_zero_tensor, net_1])


    out2_1 = tf_util.conv2d(zero_net_1, second_CNN_channels[0], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='two/adj_conv1', bn_decay=bn_decay, is_dist=True)

    out2_2 = tf_util.conv2d(out2_1, second_CNN_channels[1], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='two/adj_conv2', bn_decay=bn_decay, is_dist=True)

    out2_3 = tf_util.conv2d(out2_2, second_CNN_channels[2], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
    ...
    zero_net_2 = tf.concat(axis=-1, values=[second_zero_tensor, net_2])


    out3_1 = tf_util.conv2d(zero_net_2, third_CNN_channels[0], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='three/adj_conv1', bn_decay=bn_decay, is_dist=True)

    out3_2 = tf_util.conv2d(out3_1, third_CNN_channels[1], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='three/adj_conv2', bn_decay=bn_decay, is_dist=True)

    out3_3 = tf_util.conv2d(out3_2, third_CNN_channels[2], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
    ...
    zero_net_3 = tf.concat(axis=-1, values=[third_zero_tensor, net_3])


    out4_1 = tf_util.conv2d(zero_net_3, fourth_CNN_channels[0], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='four/adj_conv1', bn_decay=bn_decay, is_dist=True)

    out4_2 = tf_util.conv2d(out4_1, fourth_CNN_channels[1], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
                            scope='four/adj_conv2', bn_decay=bn_decay, is_dist=True)

    out4_3 = tf_util.conv2d(out4_2, third_CNN_channels[2], [2, 2],
                            padding='SAME', stride=[1, 1],
                            bn=True, is_training=is_training, weight_decay=weight_decay,
    ...
    out7 = tf_util.conv2d(tf.concat([net_1, net_2, net_3, net_4], axis=-1), 1024, [2, 2],
                          padding='SAME', stride=[1, 1],
    ...
    # CONV
    net = tf_util.conv2d(concat, 512, [2, 2], padding='SAME', stride=[1, 1],
    ...
    net = tf_util.conv2d(net, 1, [2, 2], padding='SAME', stride=[1, 1],
