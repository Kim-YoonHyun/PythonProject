...
def pairwise_distance(point_cloud):
    ...
    point_cloud = tf.squeeze(point_cloud) # 차원의 크기가 1 인 axis를 없앤다. 1이 없을 경우 그대로
    if og_batch_size == 1: # batch size 가 1 = 제일 바깥 차원의 크기가 1 인 경우
        point_cloud = tf.expand_dims(point_cloud, 0) # 차원을 하나 추가한다 (1, a, b) 로 다시 만듬

    point_cloud_transpose = tf.transpose(point_cloud, perm=[0, 2, 1]) # (1, b, a)
    point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose) # (1, a, a)
    point_cloud_inner = -2 * point_cloud_inner # (1, a, a)
...
def get_edge_feature(point_cloud, nn_idx, k=20):
    ...
    """
    og_batch_size = point_cloud.get_shape().as_list()[0] # 정수 list 또는 각 차원에 대해서 None 을 리턴
    point_cloud = tf.squeeze(point_cloud) # 크기가 1인 차원을 제거
    if og_batch_size == 1:
        point_cloud = tf.expand_dims(point_cloud, 0) # 해당 axis에 크기가 1인 차원 추가
...
def get_k_nearest_points(point_cloud, nn_idx, k):
